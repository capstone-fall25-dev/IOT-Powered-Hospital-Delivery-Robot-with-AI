...
namespace API_Powered_Hospital_Delivery_Robot.Services.ImplServices
{
    public class TaskService : ITaskService
    {
        ...
        {
            // Validate robot (từ trước)
            var robot = await _robotRepository.GetByIdAsync(createTaskDto.RobotId);
            if (robot == null) throw new InvalidOperationException("Robot not found");
            ...
            if (createTaskDto.AssignedBy.HasValue)
            {
                ...
                if (user == null) throw new InvalidOperationException("Assigned user not found");
            }
            ...
            // Tạo TaskStops (ví dụ: seq_no từ 1 đến n, start=seq1, end=last)
            foreach (var stopDto in createTaskDto.TaskStops)
            {
                ...
                await _repository.CreateTaskStopAsync(taskStop);
            }

            // Reload task với stops để response
